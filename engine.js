...
function compileCard(card) {
  const out = { cont: [], props: [], sot: [], eot: [], onEnter: null, spellOps: null, rite: null, hexTrig: null, autoBits: [], auxCont: [], auxSot: [], auxEot: [], auxOnEnter: null, auxProps: [], auxAutoBits: [], activated: [], auxActivated: [], combatTrig: [] };
  try {
    if (card.type === "hero") {
...
async function fireCombat(event, actorPi, actorLi, targetPi, targetLi) {
  ...
  for (const { tr, src } of list) {
    ...
    if (op.kind === "pulse") { PA.pulse += op.n; log(`${PA.name} gains ${op.n} Pulse (${src}).`, PA.isAI ? "ai" : ""); }
    else if (op.kind === "draw") { for (let i = 0; i < op.n; i++) drawCard(actorPi); }
    else if (op.kind === "healSelf" && actor) { const healed = Math.min(actor.dmg, op.n); actor.dmg -= healed; if (healed) log(`${src} heals ${healed}.`); }
