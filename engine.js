...
function parseOps(text) {
  ...
  for (const s of sentences(text)) {
    const parts = s.replace(/\.$/, "").split(/, then |; | and (?=gain |draw |deal |destroy |discard |lose |you gain |a Hero you control )/i);
    // try whole-sentence first (some ops contain commas)
...
function compileRecurring(text, out) {
  ...
  for (const s of sentences(text)) {
    ...
    else if ((m = s.match(new RegExp(`^At the end of each of your turns, the Hero in this lane takes ${NUM} damage`, "i")))) { out.eot.push({ op: "damage", n: +m[1], target: "laneHero" }); out.autoBits.push("end-of-turn damage"); }
  }
}
...
function compileHex(card, out) {
  ...
  if (/^When an enemy Hero attacks a Hero in this lane/i.test(t) || /^The first time each turn an enemy Hero attacks this lane/i.test(t) ||
    ...
    if ((m = t.match(new RegExp(`the defend(?:er|ing Hero)(?:'s)? gains \\+${NUM} Attack(?: and \\+${NUM} Health)?,? for that combat`, "i")))) ops.push({ op: "combatDefAtk", n: +m[1] });
    ...
    if ((m = t.match(/that attack is blocked/i))) ops.push({ op: "block" });
    ...
    ev = "oppHeroPlayed";
    if ((m = t.match(new RegExp(`enters play with -${NUM} Attack(?: and -${NUM} Health)? permanently`, "i")))) ops.push({ op: "statReduce", atk: +m[1], hp: +(m[2] || 0), target: "played" });
...
function compileCard(card) {
  const out = { cont: [], props: [], sot: [], eot: [], onEnter: null, spellOps: null, rite: null, hexTrig: null, autoBits: [], auxCont: [], auxSot: [], auxEot: [], auxOnEnter: null, auxProps: [], auxAutoBits: [], activated: [], auxActivated: [] };
  try {
    if (card.type === "hero") {
      ...
      const em = norm(card.text).match(/^When \S+ enters play, (.*?\.)/i);
...
function heroInst(cardId) {
  ...
  return { uid: uid(), cardId, permAtk: 0, permHp: 0, redAtk: 0, redHp: 0, dmg: 0, temp: [], relics: [], attacksUsed: 0, playedTurn: G ? G.gt : 0, survivedOnce: false };
}

function collectAuras(tpi, tli) {
  ...
  for (let pi = 0; pi < 2; pi++) {
    ...
    for (let li = 0; li < P.lanes.length; li++) {
      ...
      const seen = new Set();
      for (const a of L.aux) if (a && !seen.has(a.uid)) { seen.add(a.uid); sources.push({ f: fx(a.cardId), cont: "auxCont", props: "auxProps" }); }
      if (L.hero) for (const r of L.hero.relics) sources.push({ f: fx(r.cardId), cont: "cont", props: "props", equippedHere: true, forge: r.counters || 0 });
...
function startTurn() {
  ...
  for (let p = 0; p < 2; p++) for (const L of G.players[p].lanes) {
    if (L.hero) {
      if (p === pi) { L.hero.attacksUsed = 0; L.hero.extraAttacks = 0; L.hero.usedAb = {}; L.hero.relics.forEach(r => { r.usedAb = {}; }); }
...
async function runOp(op, pi, ctx) {
  ...
  switch (op.op) {
    case "pulse": P.pulse += op.n; log(`${P.name} gains ${op.n} Pulse (${ctx.sourceName || "effect"}).`); break;
    case "mortality": P.mortality += op.n; log(`${P.name} ${op.n < 0 ? "loses" : "gains"} ${Math.abs(op.n)} Mortality.`); checkWin(); break;
    ...
    case "cantAttack": {
      let targets = await resolveTargets(op, pi, ctx);
      for (const t of targets) if (heroAt(t)) {
        heroAt(t).noAttackUntil = G.gt + (op.dur || 2);
        log(`${nameOf(t)} cannot attack until turn ${Math.ceil((G.gt + (op.dur || 2)) / 2)}.`);
      }
      break;
    }
    case "sacrifice": {
      const own = heroesOf(pi);
      ...
      else t = await UI.pickHero("Choose a Hero you control to destroy", own);
...
async function activateAbility(pi, li, holder, sourceName, ab, idx, relicInst) {
  ...
  if (ab.ops) for (const op of ab.ops) await runOp(op, pi, { laneIdx: li, sourceName, relic: relicInst });
...
async function resolveTargets(op, pi, ctx) {
  const enemies = heroesOf(1 - pi), own = heroesOf(pi);
  const P = G.players[pi];
  ...
  switch (op.target) {
    ...
    case "upToAny": case "upToOwn": {
      const cands = op.target === "upToOwn" ? own : enemies.concat(own);
      const picked = [];
...
async function fireHexes(ownerPi, event, ctx) {
  ...
  for (let si = 0; si < P.slots.length; si++) {
    ...
    for (const op of f.hexTrig.ops) {
      ...
      else if (op.op === "block" && ctx.combat) ctx.combat.blocked = true;
      else if (op.op === "negate" && ctx.negate) ctx.negate.negated = true;
      else await runOp(op, ownerPi, Object.assign({ sourceName: card.name }, ctx));
    }
...
async function playHero(pi, handIdx, laneIdx) {
  ...
  const f = fx(card.id);
  if (f.onEnter) for (const op of f.onEnter) await runOp(op, pi, { laneIdx, sourceName: card.name });
...
async function playAux(pi, handIdx, laneIdx) {
  ...
  const f = fx(card.id);
  if (f.auxOnEnter) {
    for (const op of f.auxOnEnter) await runOp(op, pi, { laneIdx, sourceName: card.name });
...
async function resolveRite(pi, si, which) {
  ...
  if (ops && ops !== "manual") for (const op of ops) await runOp(op, pi, { laneIdx: s.laneIdx, sourceName: card.name });
...
async function playSpell(pi, handIdx) { // pact or incantation
  ...
  if (ops && ops !== "manual") for (const op of ops) await runOp(op, pi, { sourceName: card.name });
...
function legalTargetsFor(pi, attackerLis) {
  ...
  const enemyHeroes = heroesOf(O);
  if (free) return { heroes: enemyHeroes, face: true };
  if (!enemyHeroes.length) return { heroes: [], face: true };
  if (attackerLis.length === 1) {
    const li = attackerLis[0];
    if (heroAt({ pi: O, li })) return { heroes: [{ pi: O, li }], face: false };
    return { heroes: enemyHeroes, face: false };
  }
  // onslaught: each attacker must have empty opposing lane OR target its own opposing hero
  const shared = enemyHeroes.filter(t => attackerLis.every(li => !heroAt({ pi: O, li }) || li === t.li));
...
async function resolveAttack(pi, attackerLis, target) {
  ...
  for (const li of attackerLis) heroAt({ pi, li }).attacksUsed++;
  ...
  const defAtk = Math.max(0, effAtk(1 - pi, dLi) + (combat.defAtkMod || 0));
  ...
  if (alive.length === 1) {
    ...
    if (aAtk > defAtk) {
      ...
      if (dmg > 0) dealDamage(ctx.defender, dmg, { combat: true, killer: { pi, li: alive[0] }, sourceName: "combat" });
    ...
    if (total > defAtk) {
      ...
      if (dmg > 0) dealDamage(ctx.defender, dmg, { combat: true, killer: { pi, li: alive[0] }, sourceName: "Onslaught" });
